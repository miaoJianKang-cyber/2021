...
class FromUrlGetData(threading.Thread):
    def __init__(self):
        ...
        # 获取网页休眠时间
        self.timeSleepExeJsScript = int(self.config.get("Parameter", "timeSleepExeJsScript"))
        self.page_max_num = int(self.config.get("Parameter", "page_max_num"))
                ...
                if "product_url" in url.keys():
                    if "productDescription" not in url.keys():
                        try:
                            ...
                            self.driver.execute_script('window.open("'+ url["product_url"] +'")')
                ...
                if FlagC > self.page_max_num:
                    ...
                    for window_detail in window_list:
                        ...
                        if self.driver.current_url != 'data:,':
                            ...
                            my_query = {"product_url": self.driver.current_url}
                            new_values = {
                                "$set": {
