...
def getGongHai(driver):
    ...
    # driver.post('https://oa.globalso.com/high_seas/independent')
    ...
    for i in range(1, 4611): # ----------------------------------------------------------------------------------------->页数[已改]
            ...
            try:
                ...
                for line in tds[j].children:
                    try:  # 获取数据
                        ...
                        print('第{}页，第{}个数据格式错误'.format(i, j))
                try:   # 保存数据
                    with open('3allData20210123_5.csv', 'a', newline="") as filecsv: # ------------------------------------------->保存的文件名
        ...
        while flag_page:
            try:
                page_count = driver.find_element_by_class_name("clickpage current").text
                print("page_count",page_count)
            except:
                # print("获取页数page_count失败")
                pass
            try:
                if i == 1:
                ...
                elif i == 4 or i == 72 or i == 97 or i == 215:  # 特殊页：点下一页网站方面出错
