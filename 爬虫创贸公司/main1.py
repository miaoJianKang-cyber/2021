...
def getGongHai(driver):
    # !!!!!!!!!!招聘（日更）
    driver.get('https://oa.globalso.com/high_seas/recruitment')
    m = pymouse.PyMouse()
    time.sleep(2)
    for i in range(1, 348):
        ...
        tds = soup.find_all('td', {'colspan': '8'})
            ...
            try:
                ...
                for line in tds[j].children:
                    try:
                        ...
                        print('第{}页，第{}个数据格式错误'.format(i, j))
                try:
                    with open('1allData2.csv', 'a', newline="") as filecsv:
        ...
        driver.maximize_window()
        m.click(int(1145 + int(int(i) * 0.1)), 430)
        time.sleep(1)
        # 结束
