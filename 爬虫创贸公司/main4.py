...
def getGongHai(driver):
    driver.get('https://oa.globalso.com/high_seas/facebook') # --------------------------------------------------------->域名地址[已改]
    m = pymouse.PyMouse()
    jj=0
    for i in range(1, 4617): # ----------------------------------------------------------------------------------------->页数[已改]
        time.sleep(1)  # ----------------------------------------------------------------------------------------------->页面缓冲
        if i > 100:
            jj = 20
        main_handle = driver.current_window_handle
        # time.sleep(1)
        for k in range(1, 14): # --------------------------------------------------------------------------------------->鼠标下拉到最下面[已改]
            m.click(1900, 964) # --------------------------------------------------------------------------------------->鼠标下拉到最后所需鼠标位置[已改]
        for kk in range(1, 30):
            m.click(1900, 975) # --------------------------------------------------------------------------------------->鼠标下拉到最后再往下[已改]
        # 开始
        ...
        tds = soup.find_all('td', {'colspan': '6'})  # ----------------------------------------------------------------->facebook[]
        if(len(trs)):# 有数据
            flag = True
            data_list = []
                ...
                try:
                    ...
                    for line in tds[j].children:
                        try:
                            ...
                            print('第{}页，第{}个数据格式错误'.format(i, j))
                    try:
                        with open('4allData.csv', 'a',
                                  newline="") as filecsv:  # ------------------------------------------->保存的文件名
        ...
        driver.maximize_window()
        if flag:
            m.click(int(1140 + int(int(i) * 0.1)+int(jj)), 891) # ---------------------------------------------------------->鼠标点击下一页[已改]
        else:
            m.click(1205, 385)
